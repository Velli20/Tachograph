...
package com.velli.tachograph.googledatetimepicker;
...
public class SimpleMonthView extends View {
    ...
    public SimpleMonthView(Context context) {
        ...
        mDayTextColor = res.getColor(R.color.date_picker_date_picker_day_text_color);
        mTodayNumberColor = res.getColor(R.color.date_picker_date_picker_today_number_color);
        mMonthTitleColor = res.getColor(R.color.date_picker_date_picker_month_title_color);
        mMonthTitleBGColor = res.getColor(R.color.date_picker_date_picker_month_title_background_color);

        mStringBuilder = new StringBuilder(50);
        mFormatter = new Formatter(mStringBuilder, Locale.getDefault());

        MINI_DAY_NUMBER_TEXT_SIZE = res.getDimensionPixelSize(R.dimen.date_picker_day_number_size);
        MONTH_LABEL_TEXT_SIZE = res.getDimensionPixelSize(R.dimen.date_picker_month_label_size);
        MONTH_DAY_LABEL_TEXT_SIZE = res.getDimensionPixelSize(R.dimen.date_picker_month_day_label_text_size);
        MONTH_HEADER_SIZE = res.getDimensionPixelOffset(R.dimen.date_picker_month_list_item_header_height);
        DAY_SELECTED_CIRCLE_SIZE = res.getDimensionPixelSize(R.dimen.date_picker_day_number_select_circle_radius);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_UP:
                final SimpleMonthAdapter.CalendarDay day = getDayFromLocation(event.getX(), event.getY());
                if (day != null) {
                    onDayClick(day);
                }
                break;
     ...
     */
    private void onDayClick(SimpleMonthAdapter.CalendarDay day) {
    ...
    private class MonthViewNodeProvider extends TouchExplorationHelper<SimpleMonthAdapter.CalendarDay> {
        private final SparseArray<SimpleMonthAdapter.CalendarDay> mCachedItems = new SparseArray<SimpleMonthAdapter.CalendarDay>();
        ...
        @Override
        public void invalidateItem(SimpleMonthAdapter.CalendarDay item) {
        ...
        @Override
        protected boolean performActionForItem(SimpleMonthAdapter.CalendarDay item, int action, Bundle arguments) {
            switch (action) {
        ...
        @Override
        protected void populateEventForItem(SimpleMonthAdapter.CalendarDay item, AccessibilityEvent event) {
            event.setContentDescription(getItemDescription(item));
        }

        @Override
        protected void populateNodeForItem(SimpleMonthAdapter.CalendarDay item, AccessibilityNodeInfoCompat node) {
        ...
        @Override
        protected void getVisibleItems(List<SimpleMonthAdapter.CalendarDay> items) {
        ...
        @Override
        protected SimpleMonthAdapter.CalendarDay getItemAt(float x, float y) {
            return getDayFromLocation(x, y);
        }


        @Override
        protected int getIdForItem(SimpleMonthAdapter.CalendarDay item) {
            return item.day;
        }

        @Override
        protected SimpleMonthAdapter.CalendarDay getItemForId(int id) {
            if ((id < 1) || (id > mNumCells)) {
                return null;
            }

            final SimpleMonthAdapter.CalendarDay item;
            if (mCachedItems.indexOfKey(id) >= 0) {
                item = mCachedItems.get(id);
            } else {
                item = new SimpleMonthAdapter.CalendarDay(mYear, mMonth, id);
         ...
         */
        private void getItemBounds(SimpleMonthAdapter.CalendarDay item, Rect rect) {
         ...
         */
        private CharSequence getItemDescription(SimpleMonthAdapter.CalendarDay item) {
            if (recycle == null) {
